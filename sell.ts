...
var preco_0: number, preco_1: number, preco_2: number, preco_3: number, preco_4: number, preco_5: number, preco_6: number, preco_venda: number
...
var compra_preco_0: number, compra_quantidade_0: number, valor_quantidade_0: number
...
async function livro() {
    try {
        ...
        var posicao_0 = (livroDataSelling[0])
        var posicao_1 = (livroDataSelling[1])
        var posicao_2 = (livroDataSelling[2])
        var posicao_3 = (livroDataSelling[3])
        var posicao_4 = (livroDataSelling[4])
        var posicao_5 = (livroDataSelling[5])
        var posicao_6 = (livroDataSelling[6])
        ...
        var compra_posicao_0 = (livroDataBuying[0])
...
async function saldoCripto() {
    try {
        ...
        return saldoData.find((item) => item.currency_code == Cripto)?.available_amount ?? 0
...
async function Status(): Promise<string> {
    try {
        var url = `https://api.bitcointrade.com.br/v3/market/user_orders/${id_venda}`
...
async function estrategia() {
    try {
        ...
        controle = 0
        var saldo = await saldoCripto();
        var minimo = (20 / preco_global)
...
async function cancelarOrder() {
    try {
        ...
        await axios.delete(url,
            { headers: headers, data: { code: id_venda } },
        )
...
async function ordens() {
    try {
        const url_executed_partially = `https://api.bitcointrade.com.br/v3/market/user_orders/open?pair=${Moeda}&status=executed_partially&type=sell`
        ...
        //     id_venda = userOrdersOpenAwaiting.data.orders[0].code
